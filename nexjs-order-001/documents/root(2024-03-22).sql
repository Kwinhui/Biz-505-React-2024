...
-- 생성한 Relation 데이터를 저장할 Relation Table 생성하기
...
-- 주문-고객 테이블에는 있는 상품코드가
-- 고객정보 테이블에 없는 것이 있는가 검사
-- 여기서 NULL 데이터가 있으면 안됨    
SELECT
...
/*
 ...
 Table 의 개수를 줄이는 것을 고민해야 한다.
 이처럼 분리된 Table 을 다시 통합 하는 것을 
 제4정규화, 제5정규화 과정이라고 한다
...
/*
 ...
 1. 주문 원장에 고객코드 칼럼을 추가하고
 2. 주문-고객 Relation Table 을 참조하여
	주문번호에 맞는 고객 코드를 주문원장 Table 에 Update
...
/*
 ...
 9. SET o_ccode = return(값) 이 실행되며, o_ccode 칼럼에 값이
	변경(UPDATE) 된다.
*/

-- MYSQL 은 Update, Delete 를 2개 이상 레코드(데이터, Row)에 대하여
-- 실행할 수 없도록 초기 세팅이 되어있다.
...
(
	SELECT OC.oc_ccode
    -- 3. 그 oc_ccode와 O.o_num과 일치하는 oc_ccode를 선택해서
    FROM tbl_order_customer OC
    -- 2. tbl_order_customer 의 OC.oc_onum 이랑 O.o_num 이랑 같으면 
